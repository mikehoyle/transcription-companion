  ...
  return (
    <div className="transport">
      <div className="transport-group">
        <button type="button" className="icon-btn" title="Go to start (Home)" aria-label="Go to start" onClick={() => c.seek(0)}>
          <Icon name="start" />
        </button>
        <button type="button" className="icon-btn" title="Return to where playback started (Backspace)" aria-label="Return to where playback started" onClick={c.returnToPlayStart}>
          <Icon name="return" />
        </button>
        <button type="button" className="icon-btn" title="Back 5 s (Shift+←)" aria-label="Back 5 seconds" onClick={() => c.seekRelative(-5)}>
          <Icon name="back" />
        </button>
        <button type="button" className={`play-btn ${playing ? 'on' : ''}`} title="Play / pause (Space)" aria-label={playing ? 'Pause' : 'Play'} onClick={c.togglePlay}>
          <Icon name={playing ? 'pause' : 'play'} size={26} />
        </button>
        <button type="button" className="icon-btn" title="Forward 5 s (Shift+→)" aria-label="Forward 5 seconds" onClick={() => c.seekRelative(5)}>
          <Icon name="forward" />
        </button>
      </div>
      ...
      <div className="transport-group">
        <button type="button" className={`chip ${loop.enabled ? 'on' : ''}`} title="Toggle loop (L)" aria-pressed={loop.enabled} onClick={c.toggleLoop}>
          <Icon name="loop" size={16} /> Loop
        </button>
        <button type="button" className="chip" title="Set loop start at playhead ([)" aria-label="Set loop start at playhead" onClick={c.setLoopStartHere}>
          A
        </button>
        <button type="button" className="chip" title="Set loop end at playhead (])" aria-label="Set loop end at playhead" onClick={c.setLoopEndHere}>
          B
        </button>
        {hasLoop && (
      ...
      <div className="transport-group">
        <button type="button" className="chip" title="Add marker at playhead (M)" onClick={() => c.addMarker()}>
      ...
      <div className="transport-group right">
        ...
        <button type="button" className="icon-btn" title="Zoom in (Z)" aria-label="Zoom in" onClick={() => c.zoom(0.5)}>
          <Icon name="zoomIn" />
        </button>
        <button type="button" className="icon-btn" title="Zoom to loop / whole file (Shift+Z)" aria-label="Zoom to loop or whole file" onClick={c.zoomToLoopOrAll}>
          <Icon name="fit" />
        </button>
      </div>
    </div>
